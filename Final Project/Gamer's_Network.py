...
def create_data_structure(string_input):
    ...
    for sentence in string_input.split('.'):
        ...
        if 'is connected to' in sentence:
            network[name][0] = sentence[sentence.find(' is connected to ') + 17:].split(', ')
        if 'likes to play' in sentence:
            network[name][1] = sentence[sentence.find(' likes to play ') + 15:].split(', ')
    return network
...
def get_secondary_connections(network, user):
    ...
    for primary in network[user][0]:
        if primary in network:
            for secondary in network[primary][0]:
                if secondary not in second:
                    second.append(secondary)
    return second
...
def find_path_to_friend(network, user_A, user_B, visited=None):
    ...
    for f in network[user_A][0]:
        if f not in visited:
            visited.append(f)
            path = find_path_to_friend(network, f, user_B, visited)
            if path:
                return [user_A] + path
    return None
