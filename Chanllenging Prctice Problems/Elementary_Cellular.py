...
def cellular_automaton(string, number, generation):
    if generation == 0:
        return string
    newline = ''
    for i in range(0, len(string)):
        target = string[i-1] + string[i] + string[(i+1) % len(string)]
        newline = newline + pattern(number, target)
    return cellular_automaton(newline, number, generation-1)
