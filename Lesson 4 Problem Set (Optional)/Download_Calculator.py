...
def download_time(size, unit, bandwidth, unitband):
    units = ['kb', 'kB', 'Mb', 'MB', 'Gb', 'GB', 'Tb', 'TB']
    change = [2**10, 2**10*8, 2**20, 2**20*8, 2**30, 2**30*8, 2**40, 2**40*8]
    for i in units:
        if i == unit:
            size = size * change[units.index(i)]
    for i in units:
        if i == unitband:
            bandwidth = bandwidth * change[units.index(i)]
    sec = 1.0 * size / bandwidth
    hours = []
    minutes = []
    seconds = []
    hour = int(sec / 60 / 60)
